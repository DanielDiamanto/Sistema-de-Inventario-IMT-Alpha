...
class Alta_hardware_red extends CI_Controller {
...
{
$this->hard_red_reg_crud();
}

public function hard_red_reg_crud()
{
	$crud = new grocery_CRUD();// Aqui se crea el objeto
	//$crud->set_theme('imt');
	$crud->where('hardware.id_redes IS NOT', NULL );
	$crud->set_subject('Registro Hardware de Red');
	$crud->set_table('hardware');// Aqui se referencia la tabla de la base de datos que alimentara al objeto
	$crud->set_crud_url_path(site_url('index.php/Hardware/Alta_hardware_red/index'));
	...
	$crud->unset_columns('id_tipo_pc', 'id_printer_scaner', 'id_otros', 'ram', 'disco_duro');
	$crud->unset_fields('id_tipo_pc', 'id_printer_scaner', 'id_otros', 'ram', 'disco_duro');
