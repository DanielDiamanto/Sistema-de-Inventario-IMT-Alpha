...
class usuarios_model extends CI_Model{
    ...
    public function add($username,$password){
        $consulta=$this->db->query("SELECT username FROM usuario WHERE username LIKE '$username';");
        if($consulta->num_rows()==0){
            ...
            $consulta=$this->db->query("INSERT INTO usuario VALUES(NULL,'$username','$password');");
            if($consulta==true){
              return true;
            }else{
                return false;
